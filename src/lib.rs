...
pub type AsyncResult<R> = Result<R, BlockingError<AsyncError>>;
...
pub trait OptionalExtension<T> {
    fn optional(self) -> Result<Option<T>, BlockingError<AsyncError>>;
}
...
impl<T> OptionalExtension<T> for AsyncResult<T> {
    fn optional(self) -> Result<Option<T>, BlockingError<AsyncError>> {
        match self {
            ...
            Err(BlockingError::Error(AsyncError::Error(diesel::result::Error::NotFound))) => Ok(None),
            Err(e) => Err(e),
        }
    }
}
